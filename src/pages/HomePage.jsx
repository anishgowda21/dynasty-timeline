...
const HomePage = () => {
  ...
  const [activeTab, setActiveTab] = useState('dynasties'); // 'dynasties' or 'wars'
  ...
  // Sort wars by start year (newest first)
  const sortedWars = [...wars].sort((a, b) => b.startYear - a.startYear);

  return (
    <div>
      <div className="flex justify-between items-center mb-6">
        <h1 className="text-3xl font-bold text-dynasty-text">
          {activeTab === 'dynasties' ? 'Royal Dynasties' : 'Wars & Conflicts'}
        </h1>
        <div className="flex space-x-2">
          ...
          <button 
            onClick={() => setActiveTab === 'dynasties' 
              ? setShowAddDynastyModal(true) 
              : setShowAddWarModal(true)
            }
          ...
          >
            ...
            Add {activeTab === 'dynasties' ? 'Dynasty' : 'War'}
          </button>
        </div>
      </div>
      ...
      {activeTab === 'wars' && (
        <>
          <div className="bg-white rounded-lg shadow-md p-6 mb-8">
            <h2 className="text-xl font-bold mb-4">Wars & Conflicts Timeline</h2>
            {wars.length > 0 ? (
              <div className="h-32 flex items-center justify-center">
                <p className="text-gray-500">
                  War timeline visualization is under development. Coming soon!
                </p>
              </div>
            ) : (
              <div className="h-32 flex items-center justify-center">
                <p className="text-gray-500">
                  No wars or conflicts added yet. Add your first conflict to see it here.
                </p>
              </div>
            )}
          </div>

          <h2 className="text-2xl font-bold mb-4">All Wars & Conflicts</h2>
          {wars.length === 0 ? (
            <div className="bg-white rounded-lg shadow-md p-8 text-center">
              <p className="text-xl text-gray-500 mb-4">No wars or conflicts added yet.</p>
              <p className="text-gray-600 mb-6">Add your first war to start tracking conflicts between rulers.</p>
              <button 
      ...
      <Modal isOpen={showAddWarModal} onClose={() => setShowAddWarModal(false)}>
        <AddWarForm onClose={() => setShowAddWarModal(false)} />
      </Modal>
